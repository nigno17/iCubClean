...
void CtrlThread::run() {
    ...
    if (yarpImage!=NULL) {
        ...
        for(int i = 0; i <= 3; i++) {
            ...
            igaze->get3DPointOnPlane(camsel, pixel, plane, cartCoord[i]);
            //imgCoord[i](0) = ( cartCoord[i](1)-cartCoord[0](1) ) * pixelDensity;
            //imgCoord[i](1) = ( cartCoord[i](0)-cartCoord[0](0) ) * pixelDensity;
            imgCoord[i](0) = (cartCoord[i](1) + (2.0 / 3.0)) * pixelDensity;
            imgCoord[i](1) = (cartCoord[i](0) + 1.0) * pixelDensity;
            //cout << cartCoord[i](0) << endl;
            //cout << cartCoord[i](1) << endl;
	    //cout << cartCoord[i](2) << endl;
            //cout << "______" << endl;
            outputQuad[i] = cv::Point2f( imgCoord[i](0), imgCoord[i](1) );
            cout << outputQuad[i].x << endl;
            cout << outputQuad[i].y << endl;
            cout << "______" << endl;
        }
        cout << "+++++++++++++++++" << endl;
