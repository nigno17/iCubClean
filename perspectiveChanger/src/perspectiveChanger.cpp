...
void CtrlThread::run() {
    ...
    if (yarpImage!=NULL) {
        ...
        cout << "3x" << px.toString().c_str() << endl;
        ...
        // correction for the real icub
        cv::Point2f inputQuad2[4];
        cv::Point2f outputQuad2[4];
        cv::Mat lambda2;
        cv::Mat imgMat3;
        
        //right arm 
        /*
        inputQuad2[0] = cv::Point2f(           122,             126);
        inputQuad2[1] = cv::Point2f(           198,             120);
        inputQuad2[2] = cv::Point2f(           202,             173);
        inputQuad2[3] = cv::Point2f(           130,             181);


	outputQuad2[0] = cv::Point2f(          138,             136);
        outputQuad2[1] = cv::Point2f(          204,             132);
        outputQuad2[2] = cv::Point2f(          207,             176);
        outputQuad2[3] = cv::Point2f(          136,             183);
        */

        inputQuad2[0] = cv::Point2f(           115,             140);
        inputQuad2[1] = cv::Point2f(           182,             134);
        inputQuad2[2] = cv::Point2f(           193,             182);
        inputQuad2[3] = cv::Point2f(           113,             184);


	outputQuad2[0] = cv::Point2f(          125,             154);
        outputQuad2[1] = cv::Point2f(          182,             153);
        outputQuad2[2] = cv::Point2f(          190,             195);
        outputQuad2[3] = cv::Point2f(          120,             192);

        lambda2 = cv::getPerspectiveTransform( inputQuad2, outputQuad2 );
        warpPerspective( imgMat2, imgMat3, lambda2, imgMat2.size() );
        // END CORRECTION


        tempImg.setExternal( imgMat3.data, imgMat3.size[1], imgMat3.size[0] );
